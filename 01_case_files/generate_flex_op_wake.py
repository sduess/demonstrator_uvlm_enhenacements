...
lifting_only = False #True

case_name = 'test_flex_op'
...
alpha = 1.2209*np.pi/180
beta = 0
roll = 0
gravity = True #free_flight
cs_deflection =  -16.1955*np.pi/180
...
def generate_fem():
    ...
    for ielem in range(n_elem_main1):
        ...
        # if not np.sum(aero_node[node_counter:node_counter + n_node_elem]) >= 2:
