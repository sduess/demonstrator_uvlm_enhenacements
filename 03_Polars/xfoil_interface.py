...
def read_excel(filepath):
    ...
    out = {}
    outname = ['root', 'tip']
    for isheet, sn in enumerate([6]):
        df = pd.read_excel(filepath, sheet_name=sn, header=2, usecols=[1, 2, 3, 4])

        x_upper, y_upper = interpolate_airfoil_surface(df['XU'], df['YU'], n_points=300)
        x_lower, y_lower = interpolate_airfoil_surface(df['XL'], df['YL'], n_points=300)
...
class XFoilInterface:
    ...
    def __init__(self, configuration='Reference', **kwargs):
        ...
        self.airfoil_data = None  # dict: containing coords at 'root' and 'tip'
    ...
    def load_airfoils(self):
        ...
        self.airfoil_data = read_excel(filepath)

    def run_angle_of_attack(self, alpha):
        """
        ...
        for airfoil_name, airfoil_coords in self.airfoil_data.items():
            self.xfoil.airfoil = Airfoil(*airfoil_coords)
            self.xfoil.repanel(n_nodes=250)
...
class TestXFoil(unittest.TestCase):
    ...
    def test_load_airfoil(self):
        ...
        for name, coords in airfoil_coordinates.items():
            ...
            self.xf.repanel(n_nodes=250)

            plt.plot(coords[0], coords[1], label='source', lw=0.5, color='k')
            plt.plot(self.xf.airfoil.x, self.xf.airfoil.y, label='xfoil', lw=0.5, ls='--')
            plt.axis('equal')
            plt.legend()
    ...
    def test_run(self):
        ...
        out_data = xfi.run_polar(-3, 5, 1, save=True)
        ...
        np.testing.assert_allclose(data[:, 1], out_data['root'][:, 1], err_msg='Cl not equal to real xfoil', atol=1e-3)
        np.testing.assert_allclose(data[:, 2], out_data['root'][:, 2], err_msg='Cd not equal to real xfoil', atol=1e-3)
        np.testing.assert_allclose(data[:, 4], out_data['root'][:, 3], err_msg='Cm not equal to real xfoil', atol=1e-3)
