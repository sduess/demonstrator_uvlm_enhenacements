...
def generate_aircraft(base_case_name, polars, polar_file, alpha_deg, u_inf, **kwargs):
    alpha = alpha_deg * np.pi / 180
    alpha_deg_100 = np.ceil(alpha_deg * 100)
    case_name = base_case_name + '_p{:g}'.format(polars) + \
                '_alpha{:04g}'.format(alpha_deg_100) + '_uinf{:04g}'.format(u_inf * 10)

    print(case_name)
    route = os.path.dirname(os.path.realpath(__file__)) + '/'

    if not os.path.isdir(route + '/cases/'):
        os.makedirs(route + '/cases/')

    if not os.path.isdir(route + '/cases/' + case_name):
        os.makedirs(route + '/cases/' + case_name)

    if not os.path.isdir(route + '/output/'):
        os.makedirs(route + '/output/')
    ...
    def clean_test_files():
        fem_file_name = route + '/cases/' + case_name + '/' + case_name + '.fem.h5'
        if os.path.isfile(fem_file_name):
            os.remove(fem_file_name)

        dyn_file_name = route + '/cases/' + case_name + '/' + case_name + '.dyn.h5'
        if os.path.isfile(dyn_file_name):
            os.remove(dyn_file_name)

        aero_file_name = route + '/cases/' + case_name + '/' + case_name + '.aero.h5'
        if os.path.isfile(aero_file_name):
            os.remove(aero_file_name)

        aero_file_name = route + '/cases/' + case_name + '/' + case_name + '.nonlifting_body.h5'
        if os.path.isfile(aero_file_name):
            os.remove(aero_file_name)

        solver_file_name = route + '/cases/' + case_name + '/' + case_name + '.sharpy'
        if os.path.isfile(solver_file_name):
            os.remove(solver_file_name)

        flightcon_file_name = route + '/cases/' + case_name + '/' + case_name + '.flightcon.txt'
    ...
    def generate_fem():
        ...
        y[wn + n_node_junctions:wn + n_node_junctions + n_node_root - 1] = np.linspace(y_coord_junction,
                                                                                       y_coord_ailerons[0], n_node_root)[1:]
        ...
        x[wn + n_node_junctions:wn + n_node_main] += (abs(
            y[wn + n_node_junctions:wn + n_node_main]) - y_coord_junction) * np.tan(sweep_quarter_chord)
        ...
        index_tail_start = wn + find_index_of_closest_entry(x[wn:wn + n_node_fuselage - 1],
                                                            offset_tail_nose - offset_wing_nose)
        we += n_elem_fuselage
        ...
        with h5.File(route + '/cases/' + case_name + '/' + case_name + '.fem.h5', 'a') as h5file:
    ...
    def generate_aero_file():
        ...
        temp_chord[n_node_junctions:n_node_main] = abs(y[wn + n_node_junctions:wn + n_node_main] * np.tan(sweep_LE) - (
                    chord_root + y[wn + n_node_junctions:wn + n_node_main] * np.tan(sweep_TE)))
        ...
        temp_chord = tail_chord_root - abs(y[wn:wn + n_node_tail] * np.tan(tail_sweep_LE)) + abs(
            y[wn:wn + n_node_tail] * np.tan(tail_sweep_TE))
        ...
        with h5.File(route + '/cases/' + case_name + '/' + case_name + '.aero.h5', 'a') as h5file:
            ...
            if polars:
                polar_data = load_airfoil_tools_polars(polar_file)
                polars_group = h5file.create_group('polars')
                for i_airfoil in range(3):  # there are three airfoil
                    polars_group.create_dataset('{:g}'.format(i_airfoil), data=polar_data)
    ...
    def generate_nonlifting_body_file():
        ...
        with h5.File(route + '/cases/' + case_name + '/' + case_name + '.nonlifting_body.h5', 'a') as h5file:
            ...
            nonlifting_body_distribution_input = h5file.create_dataset('surface_distribution',
                                                                       data=nonlifting_body_distribution)
    ...
    def generate_solver_file():
        file_name = route + '/cases/' + case_name + '/' + case_name + '.sharpy'
        settings = dict()
        settings['SHARPy'] = {'case': case_name,
                              'route': route + '/cases/' + case_name + '/',
                              'flow': flow,
                              'write_screen': 'on',
                              'write_log': 'on',
    ...
    smain.main(['', route + '/cases/' + case_name + '/' + case_name + '.sharpy'])


if __name__ == '__main__':
    ######################################
    base_case_name = 'flex_op_polars'
    polars = True
    polar_file = './xf-naca0018-il-50000.csv'
    alpha_deg = 2.8
    u_inf = 40

    generate_aircraft(base_case_name, polars, polar_file, alpha_deg, u_inf)
    ######################################
